...
outfile_error = 'dataset_error_ANN_puntos_no_incluidos.xlsx';
...
% La base de datos utilizada para entrenar la ANN fue generada con:
%
%   T = -25:2:85 °C
%   G = 100:25:1200 W/m^2
%
% Para esta evaluación se seleccionan valores de T y G que NO
% pertenecen a dicha malla, pero permanecen dentro del rango
% utilizado para el entrenamiento de la ANN.

% Temperaturas no incluidas en la malla original
T_vec = [16 26 36 46 54];

% Irradiancias no incluidas en la malla original
G_vec = [212.5 312.5 412.5 512.5 612.5 712.5 ...
         812.5 912.5 1012.5 1112.5 1187.5];
...
for it = 1:nT
    ...
    residual = @(I,Va_loc,G_loc) ...
        + gamma_IL * Npar * ...
          (Isc/Apanel + Ki*(Tcell - Tref)/Apanel) * ...
          (G_loc/Gref) * Apanel ...
        - (Npar*Is*(Tcell/Tref)^3 .* ...
          exp((q*Egref)/(k1*Tref) - (q*Eg)/(k1*Tcell))) ...
          .* (exp((Va_loc + I.*Rspanel*(Nser/Npar)) ./ ...
          (Nser*n*Ncell*(k1*Tcell/q))) - 1) ...
        - (Va_loc + I.*Rspanel*(Nser/Npar)) ./ ...
          (Rshpanel*(Nser/Npar)) .* (G_loc/Gref) ...
        - I;
...
DataError = table(T_col, G_col, Vmpp_col, Impp_full_col, ...
                  Pmpp_col, Impp_ANN_col, ...
                  ErrAbs_col, ErrRel_col, ...
    'VariableNames', ...
    {'T','G','Vmpp','Impp_full','Pmpp','Impp_ANN', ...
     'ErrAbs_A','ErrRel_pct'});
...
fprintf('\n');
fprintf('============================================================\n');
fprintf(' ERROR DE LA ANN EN PUNTOS NO INCLUIDOS EN LA MALLA ORIGINAL\n');
fprintf('============================================================\n');

fprintf('Numero total de puntos evaluados = %d\n', N);
fprintf('Error relativo medio             = %.4f %%\n', mean_err);
fprintf('Error relativo maximo            = %.4f %%\n', max_err);
fprintf('Error relativo minimo            = %.4f %%\n', min_err);


%% ================== PUNTO DE ERROR MAXIMO ==================

[~, idx_max] = max(ErrRel_col);

fprintf('\n==== PUNTO DE ERROR RELATIVO MAXIMO ====\n');

fprintf('T              = %.1f °C\n', T_col(idx_max));
fprintf('G              = %.1f W/m^2\n', G_col(idx_max));
fprintf('Impp modelo    = %.6f A\n', Impp_full_col(idx_max));
fprintf('Impp ANN       = %.6f A\n', Impp_ANN_col(idx_max));
fprintf('Error absoluto = %.6f A\n', ErrAbs_col(idx_max));
fprintf('Error relativo = %.4f %%\n', ErrRel_col(idx_max));


%% ================== ERROR PROMEDIO POR TEMPERATURA ==================

fprintf('\n==== ERROR RELATIVO POR TEMPERATURA ====\n');
...
for it = 1:nT
    ...
    fprintf(['T = %5.1f °C --> Error medio = %.4f %% | ' ...
             'Error maximo = %.4f %%\n'], ...
        Tnow, ...
        mean(ErrRel_col(idxT)), ...
        max(ErrRel_col(idxT)));
...
if plot_error_vs_G
    ...
    for it = 1:nT
        ...
        plot(G_col(idxT), ErrRel_col(idxT), ...
            '-o', ...
            'LineWidth', 1.3, ...
    ...
    title(['Error relativo de la ANN en puntos no incluidos ' ...
...
if plot_error_surface
    ...
    title(['Superficie de error relativo de la ANN en puntos ' ...
           'no incluidos en la malla original']);
...
if plot_Impp_compare
    ...
    % Comparar Impp del modelo PV-NREL contra Impp estimada por la ANN
    for it = 1:nT

        Tnow = T_vec(it);
        idxT = (T_col == Tnow);

        plot(G_col(idxT), Impp_full_col(idxT), ...
            '-o', ...
            'LineWidth', 1.4, ...
            'DisplayName', sprintf('Modelo PV-NREL, T=%g °C', Tnow));

        plot(G_col(idxT), Impp_ANN_col(idxT), ...
            '--s', ...
            'LineWidth', 1.2, ...
    ...
    title(['Comparación de I_{mpp} entre el modelo PV-NREL y la ANN ' ...
           'en puntos no incluidos en la malla original']);
